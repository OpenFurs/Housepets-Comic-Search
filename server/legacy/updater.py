...
def main():
    ...
    two_latest_comics = [comics["href"].split(
        "/")[-2] for comics in latest_comics[-2:]]
    ...
    latest_index = int(housepets_db.hget(
        f"comics:{two_latest_comics[0]}", "index")) + 1

    comic = hp.get_comic_metadata(latest_comics[-1], index=latest_index)
    comic.get("comic").update(
        {"chapter": latest_chapter, "year": current_year})
